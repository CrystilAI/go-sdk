...
func (a Attribution) validate() error {
	if a.ParentID == "" {
		return errors.New("crystil: parent ID required")
	}
	if len(a.ParentID) > 100 {
		return errors.New("crystil: parent ID exceeds 100 characters")
	}
	if a.ParentName != nil && len(*a.ParentName) > 100 {
		return errors.New("crystil: parent name exceeds 100 characters")
	}
	if a.SubsidiaryID != nil {
		if len(*a.SubsidiaryID) > 100 {
			return errors.New("crystil: subsidiary ID exceeds 100 characters")
		}
	}
	if a.SubsidiaryName != nil && len(*a.SubsidiaryName) > 100 {
		return errors.New("crystil: subsidiary name exceeds 100 characters")
	}
	return nil
}
